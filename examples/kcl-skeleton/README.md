...
Minimal repo exercising the KCL-based configuration layer ([design](../../docs/redesign/design.md)). The `kcl.mod` file
...
- `prototypes/hello` is a regular myks prototype with an inline (hermetic) helm chart; per-app resolved values from the
  tree reach ytt templates via a generated data-values file
...
Smart Mode is not supported in KCL mode: a plain `myks render` warns and renders everything.

...
// Config holds all naming conventions and path configuration for myks.
// These values can be customised via the naming-conventions config key.
// All fields carry default values via the creasty/defaults tag.
type Config struct {
	...
	// Ytt package step directory name (deprecated, not used)

...
func (m *migrator) writeProtoK(proto string) error {
	...
	b := &kclWriter{todo: computedPaths(m.protoComputed[proto])}
...
type protoSchemaPlan struct {
	...
	elements [][]string        // the array paths with an element schema, in generation order
...
func (p *protoSchemaPlan) nameNested(path []string, values map[string]any) {
	for _, key := range p.attributes(path, values) {
		...
		if p.structuredElement(childPath) {
			p.items[pathKey(childPath)] = p.claimName(childPath)
			p.elements = append(p.elements, childPath)
		}
	}
}
...
func (p *protoSchemaPlan) render(b *kclWriter, proto string) {
	...
	for _, path := range p.elements {
		p.renderElementSchema(b, path)
	}
}
	...
	for key, value := range out {
		...
		case []any:
			if _, ok := p.items[pathKey(childPath)]; ok {
				out[key] = completeElements(typed, schemaDefaults(p.schema.nodeAt(childPath).Items))
			}
		}
	}
	return out
...
// completeElements copies a list, filling into every element the defaults it does not state.
func completeElements(list []any, defaults map[string]any) []any {
...
// renderElementSchema writes the schema of one array's element: the fields the ytt schema
// fixed for it, with its defaults. Only the element's own properties are typed; a structured
// value inside one stays a literal default, which is what the element's index signature takes.
func (p *protoSchemaPlan) renderElementSchema(b *kclWriter, path []string) {
	items := p.schema.nodeAt(path).Items
	defaults := schemaDefaults(items)
	b.WriteString("\n")
	b.printf("schema %s:\n", p.items[pathKey(path)])
	b.WriteString("    [...str]: any\n")
	for _, key := range slices.Sorted(maps.Keys(defaults)) {
		b.printf("    %s?: %s = ", key, kclType(items.Properties[key]))
		writeKclValue(b, defaults[key], 4, false, "")
		b.WriteString("\n")
	}
}
...
type kclWriter struct {
	b   strings.Builder
	err error
	// todo holds the dotted paths of the values ytt computed for this file, which are written
	// as literals and marked so the hand-finish can find them (markComputed).
	todo map[string]bool
}

// markComputed writes the TODO above a value ytt computed, if this is one.
func (w *kclWriter) markComputed(path, pad string) {
	if !w.todo[path] {
		return
	}
	w.printf("%s# TODO(myks migrate): ytt computed this; write the KCL derivation (docs/migration.md).\n", pad)
}

// computedPaths turns the reported paths of one or more scopes into a lookup set.
func computedPaths(paths ...[]string) map[string]bool {
	todo := map[string]bool{}
...
func (m *migrator) renderEnvK(node, parent *migNode) (string, error) {
	b := &kclWriter{todo: computedPaths(node.envComputed)}
...
func (m *migrator) renderAppK(node *migNode, name string) (string, error) {
	...
	if declared {
		b.todo = computedPaths(m.declarationComputed(node, name, app.proto))
	} else {
		proto := protoOf(node, name)
		b.todo = computedPaths(node.protoComputed[proto], node.appComputed[name])
	}
